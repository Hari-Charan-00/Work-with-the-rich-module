...
def main():
    ...
    for partner_id, partner_name in partners.items():
        console.rule(f"[bold blue]Processing Partner: {partner_name}[/bold blue]")
        ...
        with Progress(
            ...
            task = progress.add_task("[cyan]Fetching Clients...", start=True)
            clients = fetch_clients(access_token, partner_id, URL)
            progress.update(task, completed=True)
        
        console.print(f"Total clients fetched: [green]{len(clients)}[/green]\n")
        ...
        for client_id, client_name in clients.items():
            ...
            if noc_name in ["SRO1", "SRO2", "Vistara NOC"]:
                with Progress(
                    ...
                    device_task = device_progress.add_task(f"[cyan]Fetching devices for {client_name}...", start=True)
                    devices = fetch_devices(access_token, client_id, client_name, partner_id, URL)
                    device_progress.update(device_task, description=f"[green]Fetched devices for {client_name}[/green]", completed=True)

                if devices:
                    console.print(f"[bold green]✅ Devices fetched for client: {client_name}[/bold green]\n")
                    ...
                    # Print warning only if no devices were found
                    console.print(f"[yellow]⚠️ No devices found for client: {client_name}[/yellow]\n")
                    continue

    if all_devices_info:
        df = pd.DataFrame(all_devices_info)
        df.to_excel("client_devices_report.xlsx", index=False)
        console.print("\n[bold green]✅ Excel file 'client_devices_report.xlsx' created successfully.[/bold green]")
    else:
        console.print("\n[bold yellow]⚠️ No device data found to export.[/bold yellow]")
